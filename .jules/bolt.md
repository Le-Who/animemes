...
## 2025-02-24 - [Animation Loop GC & Layout Trashing]
**Learning:** Calling `.toLocaleString()` inside a `requestAnimationFrame` loop causes implicit locale resolution and creates excessive garbage collection pressure per frame. Furthermore, updating DOM text indiscriminately triggers unnecessary browser repaints.
**Action:** Instantiate `Intl.NumberFormat` outside animation loops and guard DOM updates (`textContent`) with a simple value equality check to maintain stable frame rates.
