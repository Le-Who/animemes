...
## 2024-11-20 - [Performance: Intl.NumberFormat vs toLocaleString]
**Learning:** Repeatedly calling `toLocaleString()` within a `requestAnimationFrame` loop creates significant garbage collection overhead and browser repaints. A shared `Intl.NumberFormat` instance is 7-9x faster.
**Action:** Cache and reuse `Intl.NumberFormat` instances for high-frequency formatting, and strictly guard DOM updates (like `textContent`) behind value-change checks to prevent redundant repaints.
