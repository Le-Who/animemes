...
## 2024-05-24 - Optimize requestAnimationFrame loops
**Learning:** Calling `.toLocaleString()` inside a `requestAnimationFrame` loop causes significant garbage collection pressure and implicit locale resolution overhead. Unconditional DOM element `textContent` updates cause unnecessary browser repaints.
**Action:** Instantiate `Intl.NumberFormat` outside the loop and use its `.format()` method. Guard DOM updates with a value-change check (`if (currentVal !== lastVal)`) to skip redundant repaints.
