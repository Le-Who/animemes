...
## 2024-05-28 - [Animation Loop Optimization]
**Learning:** Calling `.toLocaleString()` on numbers inside `requestAnimationFrame` creates unnecessary object allocations, causing garbage collection pressure. Additionally, updating DOM text nodes with identical string values can trigger redundant browser repaints.
**Action:** Instantiate `Intl.NumberFormat` outside the loop to reuse it, and add a guard variable (`lastRenderedVal`) to skip DOM updates when the value hasn't changed.
