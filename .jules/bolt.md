...
## 2024-06-30 - HTTP Connection Pooling Optimization
**Learning:** Making repeated API requests to the same host using `requests.get()` inside a loop creates a new TCP/TLS connection for each request, adding significant latency overhead.
**Action:** Always use `requests.Session()` when making multiple API calls to the same domain to reuse connections and reduce request time drastically.
