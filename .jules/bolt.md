...
## 2024-05-24 - Animation Loop GC and Repaint Optimizations
**Learning:** Calling .toLocaleString() inside requestAnimationFrame causes severe GC pressure and implicit locale resolution overhead. Frequent textContent updates without value-change checks trigger unnecessary browser repaints.
**Action:** Instantiate Intl.NumberFormat outside the loop and use .format(). Guard DOM updates with an if statement to only write when the value actually changes.
