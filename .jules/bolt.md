...
## 2024-05-15 - [Connection Reuse with requests.Session]
**Learning:** Making multiple HTTP requests in a loop using bare `requests.get()` incurs high latency because a new TCP/TLS connection is established for each request. Reusing connections with `requests.Session()` drastically reduces total execution time.
**Action:** Always prefer `requests.Session()` for repeated HTTP requests to the same host instead of `requests.get()`.
