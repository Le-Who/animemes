...
## 2026-03-30 - [Animation Loop Optimization]
**Learning:** Using `toLocaleString()` inside a `requestAnimationFrame` loop creates massive garbage collection overhead. Redundant `textContent` updates also cause unnecessary browser repaints.
**Action:** Instantiate `Intl.NumberFormat` outside animation loops and guard DOM updates with a value-change check to optimize rendering performance.
