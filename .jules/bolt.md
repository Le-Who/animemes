...
## 2026-05-01 - [Animation Loop Optimization: Number Formatting]
**Learning:** Calling `.toLocaleString()` inside a `requestAnimationFrame` loop creates implicit locale resolution overhead and garbage collection pressure. Additionally, updating `textContent` every frame even when the formatted value hasn't changed causes unnecessary browser repaints.
**Action:** Instantiate `Intl.NumberFormat` outside the animation loop and use its `.format()` method. Guard DOM updates with a value-change check to minimize repaints.
