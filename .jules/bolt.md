...
**Learning:** Calling `Number.prototype.toLocaleString()` inside a high-frequency animation loop (like `requestAnimationFrame`) is slow because it instantiates a new `Intl.NumberFormat` object internally on every call.
**Action:** Instantiate a single `Intl.NumberFormat` object and reuse its `.format()` method to avoid continuous memory allocation and garbage collection.
