...
## 2024-10-25 - [JS Performance: Number Formatting]
**Learning:** Calling `.toLocaleString()` continuously in high-frequency animation loops (like `requestAnimationFrame`) is expensive because it instantiates a new number formatter object under the hood each time.
**Action:** Instantiate a single `Intl.NumberFormat` object and reuse its `.format()` method to avoid continuous object allocation and overhead in tight loops.
