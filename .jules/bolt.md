...
## 2026-06-13 - Cache Intl.NumberFormat for animation loops
**Learning:** Using `.toLocaleString()` inside a tight `requestAnimationFrame` loop creates a new `Intl.NumberFormat` instance on every frame, which is extremely slow (up to 7x slower).
**Action:** Cache the `new Intl.NumberFormat()` instance outside of the loop and reuse `.format()` for high-frequency updates.
