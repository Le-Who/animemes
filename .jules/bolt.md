...
## 2026-07-22 - Number Formatting Performance
**Learning:** Calling `.toLocaleString()` repeatedly inside a `requestAnimationFrame` loop is computationally expensive. Caching a `new Intl.NumberFormat()` instance significantly reduces execution time (~85% faster in microbenchmarks).
**Action:** Use a cached `Intl.NumberFormat()` instead of `.toLocaleString()` for formatting numbers in hot paths or animation loops.
