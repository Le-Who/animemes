...
## 2024-05-24 - [Backend Performance: Connection Pooling]
**Learning:** Repeatedly making HTTP requests in a loop using bare `requests.get()` opens and closes a new TCP/TLS connection every time, which introduces massive overhead.
**Action:** Always prefer `requests.Session()` when making multiple requests to the same host to reuse connections and reduce latency.
