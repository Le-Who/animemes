...
**Learning:** Calling `.toLocaleString()` inside a `requestAnimationFrame` loop creates implicit locale resolution overhead and garbage collection pressure every frame. Unconditional DOM updates, even with `textContent`, can trigger unnecessary browser repaints.
**Action:** Instantiate `Intl.NumberFormat` outside the animation loop. Cache the formatted value and guard DOM updates to only occur when the value actually changes.
