...
**Learning:** Making repeated API calls to the same host using bare `requests.get()` adds significant TCP/TLS handshake latency for each call.
**Action:** Use `requests.Session()` to enable connection pooling and reuse the underlying connection, yielding a massive performance boost.
