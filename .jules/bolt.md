...
## 2024-10-25 - [Animation Loop: Object Allocation & Layout Thrashing]
**Learning:** Instantiating `Intl.NumberFormat` inside a `requestAnimationFrame` loop creates unnecessary garbage and overhead. Updating `textContent` every frame, even if the value hasn't changed, triggers redundant browser layout/paint cycles.
**Action:** reuse formatters at module scope and check `currentVal !== lastVal` before touching the DOM in animation loops.
