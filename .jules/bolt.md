...
**Learning:** `Number.prototype.toLocaleString()` inside high-frequency loops (like `requestAnimationFrame`) causes performance degradation due to implicit locale resolution and garbage collection overhead. Furthermore, updating DOM text indiscriminately causes unnecessary repaints.
**Action:** Instantiate `Intl.NumberFormat` outside the loop and use its `.format()` method. Guard DOM updates (`textContent`) with a value-change check to prevent unnecessary browser repaints.
