...
## 2024-05-30 - Connection Pooling Optimization
**Learning:** Making multiple API requests to the same host using bare `requests.get()` causes significant overhead due to repeated TCP connection establishment and TLS handshakes.
**Action:** Always prefer `requests.Session()` when making repeated or loop-based requests to the same backend service to enable connection pooling and reduce overall latency.
