...
## 2026-06-15 - [DOM Performance: Redundant formatting and DOM writes]
**Learning:** Calling `.toLocaleString()` and updating `.textContent` inside a `requestAnimationFrame` loop, even if the value hasn't changed, introduces significant CPU and DOM overhead.
**Action:** Always cache the last formatted string and value, and only update the DOM/reformat if the computed value is genuinely different from the previous frame.
