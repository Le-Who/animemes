...
## 2024-10-25 - [Animation Performance: toLocaleString and DOM repaints]
**Learning:** Calling `.toLocaleString()` per frame adds GC overhead and implicit locale resolution. Unguarded DOM writes cause unnecessary repaints.
**Action:** Instantiate `Intl.NumberFormat` outside the loop and guard DOM updates (`textContent`) with a value-change check.
