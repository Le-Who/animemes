...
**Learning:** Calling `toLocaleString()` inside a `requestAnimationFrame` loop causes hidden GC overhead. Updating `textContent` on every frame even when the value hasn't changed triggers unnecessary browser repaints.
