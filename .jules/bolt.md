...
## 2024-10-25 - [Animation Loop: Number Formatting]
**Learning:** `number.toLocaleString()` inside a `requestAnimationFrame` loop creates excessive garbage and CPU overhead. Reusing a single `Intl.NumberFormat` instance and only updating the DOM when the displayed value changes prevents unnecessary layout work.
**Action:** Hoist formatters out of loops and check `newValue !== oldValue` before touching the DOM in animations.
