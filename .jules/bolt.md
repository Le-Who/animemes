...
## 2026-02-21 - [JS Performance: Intl.NumberFormat vs toLocaleString]
**Learning:** `Intl.NumberFormat` is significantly faster than `toLocaleString()` when called repeatedly (e.g., in animation loops). `toLocaleString()` creates a new formatter instance internally each time, while reusing a `new Intl.NumberFormat()` instance avoids this overhead (measured 7x speedup).
**Action:** Always instantiate `Intl.NumberFormat` once and reuse it for formatting numbers in hot paths.
