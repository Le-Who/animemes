...
## 2024-05-23 - [Animation Loop Optimizations]
**Learning:** Calling `.toLocaleString()` inside `requestAnimationFrame` causes implicit locale resolution and excessive garbage collection (GC) pressure. Updating `.textContent` unconditionally triggers unnecessary browser repaints even if the visual value hasn't changed.
**Action:** Instantiate `Intl.NumberFormat` outside animation loops and use its `.format()` method. Guard DOM updates with a value-change check (`if (currentValue !== lastValue)`) to prevent unnecessary repaints.
