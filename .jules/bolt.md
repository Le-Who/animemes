...
## 2024-05-23 - Avoid toLocaleString() in requestAnimationFrame loops
**Learning:** Calling `toLocaleString()` on numbers inside high-frequency loops (like `requestAnimationFrame`) causes unnecessary garbage collection and performance overhead because it instantiates a new formatter object every time.
**Action:** Cache and reuse a single `Intl.NumberFormat` instance globally (e.g., `new Intl.NumberFormat()`) for repeated number formatting instead.
