...
## 2024-05-25 - [Formatting Performance: toLocaleString vs Intl.NumberFormat]
**Learning:** Calling `Number.prototype.toLocaleString()` inside high-frequency loops (like `requestAnimationFrame`) is slow because it re-instantiates the formatter every time.
**Action:** Cache and reuse a single `Intl.NumberFormat` instance for repeated number formatting.
