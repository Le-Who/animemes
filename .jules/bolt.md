...
## 2026-07-21 - [DOM Performance: toLocaleString in Animation Loop]
**Learning:** Calling `.toLocaleString()` inside a high-frequency animation loop (like `requestAnimationFrame`) is significantly slower than instantiating `Intl.NumberFormat` once and using its `.format()` method. This overhead can cause frame drops.
**Action:** Always cache `new Intl.NumberFormat()` when formatting numbers repeatedly, especially in animation loops or large iterations.
