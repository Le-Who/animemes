...
## 2024-05-24 - Connection Pooling Optimization
**Learning:** Making repeated API requests to the same host using `requests.get()` inside a loop incurs significant overhead due to repeatedly establishing TCP/TLS connections.
**Action:** Always prefer `requests.Session()` over bare `requests.get()` when making multiple or repeated HTTP requests to the same host to reuse connections and reduce latency.
