...
## 2025-02-18 - [Animation Performance: Garbage Collection in requestAnimationFrame]
**Learning:** Calling `.toLocaleString()` inside a high-frequency `requestAnimationFrame` loop creates unnecessary temporary objects, causing garbage collection pressure and frame drops. Furthermore, blindly updating `textContent` even if the rounded value hasn't changed triggers unnecessary layout calculations.
**Action:** Instantiate `Intl.NumberFormat` outside the loop and use its `.format()` method. Guard `textContent` updates with a check to ensure the value has actually changed before updating the DOM.
