...
## 2024-05-24 - [JS Performance: Intl.NumberFormat vs toLocaleString]
**Learning:** Calling `.toLocaleString()` inside a high-frequency loop like `requestAnimationFrame` creates significant overhead because a new formatter instance is created implicitly on every call.
**Action:** Cache and reuse a single `new Intl.NumberFormat()` instance for number formatting inside loops or frequent updates.
