...
## 2026-05-19 - Intl.NumberFormat optimization
**Learning:** Calling `Number.prototype.toLocaleString()` inside hot loops like `requestAnimationFrame` creates unnecessary object allocation and parsing overhead.
**Action:** Instantiate a single `Intl.NumberFormat` object and reuse its `.format()` method, which is significantly faster for repeated formatting.
