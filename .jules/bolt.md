...
## 2024-11-28 - [Animation Loop Optimization]
**Learning:** Calling `.toLocaleString()` per frame inside `requestAnimationFrame` creates implicit locale resolution and garbage collection overhead, causing frame drops.
**Action:** Always instantiate `Intl.NumberFormat` outside the loop and use `.format()`. Additionally, guard DOM updates (like `textContent`) with a value-change check to prevent unnecessary browser repaints.
