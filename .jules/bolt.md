...
## 2024-05-24 - API Request Connection Pooling
**Learning:** Reusing HTTP connections with `requests.Session()` avoids the TCP handshake overhead on every single API request, significantly reducing the total execution time for bulk data-fetching tasks.
**Action:** Always consider utilizing `requests.Session()` when making multiple requests to the same host in sequence.
