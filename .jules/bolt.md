...
**Learning:** Making repeated API requests to the same host using bare `requests.get()` in a loop establishes a new TCP connection and TLS handshake every time, which drastically hurts backend script performance.
**Action:** Use `requests.Session()` to enable HTTP connection pooling. Pass the session object to the fetching function to reuse the underlying connection for all API calls in the loop.
