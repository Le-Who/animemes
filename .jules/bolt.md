...
## 2026-03-23 - [Animation Loop Performance: GC and Repaints]
**Learning:** Using `toLocaleString()` inside a `requestAnimationFrame` loop creates significant garbage collection overhead. Additionally, assigning the same value to `textContent` redundantly triggers unnecessary browser repaints.
**Action:** Instantiate `Intl.NumberFormat` outside of animation loops, and guard DOM updates with a value-change check to minimize repaints.
