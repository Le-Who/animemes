...
## 2024-06-24 - Screen Reader Noise from Visual Keyboard Hints
**Learning:** Purely visual keyboard shortcut hints (like arrows or enter symbols) create unnecessary and confusing audio noise for screen reader users when embedded inside interactive elements.
**Action:** Always add `aria-hidden="true"` to visual-only keyboard hints (like `.key-hint` spans/divs) so they are hidden from assistive technologies while remaining visible to sighted keyboard users.
