...
## 2026-02-06 - Screen Reader Noise from Visual Keyboard Hints
**Learning:** Purely visual keyboard shortcut hints (like "←" or "↵" symbols) can create disruptive noise for screen reader users when read aloud verbatim, as they don't provide meaningful semantic value beyond the visual cue for sighted users.
**Action:** Always add `aria-hidden="true"` to structural or decorative elements like `.key-hint` that contain symbol characters solely for visual instruction.
