...
## 2024-05-28 - Restored Keyboard Focus on Round Transition
**Learning:** When hiding or removing an active element (like the "Next" button in our game), browser focus drops to the `<body>`. This breaks keyboard navigation for users relying on Tab/Enter.
**Action:** Always programmatically restore focus (e.g., `.focus()`) to the next logical interactive element (e.g., the first card) when transitioning UI states.
