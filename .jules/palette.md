...
## 2024-05-24 - Screen Reader Noise from Keyboard Hints
**Learning:** Purely visual keyboard shortcut hints (like "←", "→", or "↵" symbols) are read aloud by screen readers, creating disruptive symbol noise for users who navigate via keyboard/screen reader and already know how to interact with the elements.
**Action:** Always add `aria-hidden="true"` to elements that solely display visual keyboard shortcuts, ensuring screen readers announce only the semantic meaning (e.g., "Vote for X") rather than confusing symbol characters.
