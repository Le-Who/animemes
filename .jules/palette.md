...
## 2026-03-31 - Screen Reader Noise from Visual Hints
**Learning:** Decorative visual hints, like keyboard shortcut icons (←, →, ↵), are often read out loud by screen readers as raw, confusing symbols ('leftward arrow', 'downwards arrow with corner leftwards'). This creates an inaccessible and noisy experience for visually impaired users.
**Action:** Always add `aria-hidden="true"` to visual-only elements that provide redundant information or are strictly decorative, to hide them from the accessibility tree.
