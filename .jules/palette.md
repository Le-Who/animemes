...
## 2026-10-24 - Live Region for Game State
**Learning:** Visual-only feedback (animations, color changes) for game results is invisible to screen readers. Adding a visually hidden `aria-live="polite"` region ensures blind users receive immediate, spoken feedback on win/loss states.
**Action:** Implement a central `announce()` helper that updates a hidden live region for all critical status changes.
