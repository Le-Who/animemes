...
## 2026-05-24 - Focus Restoration on Hidden Elements
**Learning:** When hiding an active element (like a "Next" button after a click), the browser drops keyboard focus back to the `<body>`. This completely breaks the keyboard navigation flow for screen reader and keyboard-only users.
**Action:** Always programmatically restore focus (e.g., using `.focus()`) to the next logical interactive element when the currently focused element is removed or hidden from the DOM.
