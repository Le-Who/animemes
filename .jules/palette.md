...
## 2024-05-18 - Hide raw keyboard symbols from screen readers
**Learning:** Visual hints for keyboard shortcuts like arrow keys (←, →) or return (↵) can be confusing for screen reader users, who will just hear raw symbol names out of context.
**Action:** Always add `aria-hidden="true"` to elements that contain these visual shortcut hints to keep the screen reader experience clean.
