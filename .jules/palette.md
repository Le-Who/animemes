...
## 2026-05-23 - Interactive Element Focus States
**Learning:** Keyboard-only users can completely lose their place if interactive elements like buttons lack a visible focus state, making primary calls-to-action (like "Next") "invisible" during navigation. Furthermore, screen readers need `aria-live` on dynamically updating score containers to announce changes.
**Action:** Always provide explicit `:focus-visible` styles for all interactive buttons and use `aria-live="polite"` on dynamically changing scoreboards.
