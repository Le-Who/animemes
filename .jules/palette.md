...
## 2024-10-24 - Screen Reader Unfriendly Characters
**Learning:** Decorative visual hints like keyboard shortcut icons (←, →, ↵) are read out loud by screen readers as raw symbols, which can be very confusing for users relying on assistive technologies.
**Action:** Always include `aria-hidden="true"` on decorative key hints or symbols to hide them from screen readers while keeping them visible for sighted users.
