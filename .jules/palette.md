...
## 2024-06-14 - Hide Keyboard Hints from Screen Readers
**Learning:** Purely visual keyboard shortcut hints (like arrows or enter symbols) can create disruptive noise for screen reader users if left exposed, confusing them during navigation.
**Action:** Always add `aria-hidden="true"` to decorative keyboard hint elements so they are ignored by screen readers while remaining visible to sighted keyboard users.
