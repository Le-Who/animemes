...
## 2026-03-16 - Restoring Keyboard Focus on Dynamic UI Updates
**Learning:** When hiding or completely replacing an active interactive element (like a "Next" button concluding a state transition), focus natively drops to the `<body>`. This completely breaks keyboard navigation for screen-reader or keyboard-only users, forcing them to tab continuously through the document again.
**Action:** Programmatically restore focus to the next logical interactive element (e.g., using `.focus()`) within transition functions to ensure a continuous and accessible user flow.
