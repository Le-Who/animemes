...
## 2026-04-06 - Screen Reader Optimization for Visual Hints
**Learning:** Decorative text elements, like keyboard shortcut symbols (←, →, ↵), are read aloud by screen readers as raw, confusing symbols, which degrades the experience for users relying on assistive technology.
**Action:** Always apply `aria-hidden="true"` to visual-only hints so screen readers ignore them, ensuring a clean and focused auditory experience.
