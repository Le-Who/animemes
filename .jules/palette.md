...
## 2024-04-10 - Screen Reader Experience with Decorative Symbols
**Learning:** Decorative visual hints, such as keyboard shortcut icons (←, →, ↵), can be read aloud by screen readers as raw, confusing symbols (e.g., "leftwards arrow").
**Action:** Always add `aria-hidden="true"` to purely visual symbols so they are hidden from the accessibility tree, especially when their parent container already has an accessible label.
