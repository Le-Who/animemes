...
## 2025-03-24 - HTTP Parameter Pollution via String Interpolation
**Vulnerability:** API requests in `update.py` were manually constructed using f-strings to append URL parameters (`&tags={tag}`). If a tag included reserved URI characters like `&` or `=`, it could override subsequent query parameters (HTTP Parameter Pollution) or alter the target API action.
**Learning:** Always use the `params` keyword argument in `requests.get()` when handling dynamic or user-controlled query strings. This delegates URL encoding to the requests library, which safely percent-encodes all reserved characters.
**Prevention:** Avoid manual URL string interpolation for endpoints that take external data. Enforce the use of dictionary-based parameter serialization.
