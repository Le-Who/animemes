...
## 2024-05-24 - API Parameter Injection via f-strings
**Vulnerability:** External input was directly concatenated into API request URLs using f-strings in update.py, allowing potential API parameter injection or SSRF.
**Learning:** Python's requests library automatically handles URL encoding, but only when parameters are passed securely via the params dictionary. String concatenation bypasses this safety mechanism.
**Prevention:** Never use f-strings or string concatenation to build URLs with dynamic query parameters. Always pass parameters via the params argument in requests.get() or requests.post().
