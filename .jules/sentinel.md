...
## 2025-04-13 - HTTP Parameter Pollution in API Requests
**Vulnerability:** `update.py` constructed API URLs using unsafe f-string interpolation (`f"&tags={tag}"`), allowing HTTP Parameter Pollution if `tag` contained malicious query parameters.
**Learning:** String interpolation for URLs fails to encode special characters, allowing attackers to inject arbitrary parameters or overwrite existing ones. Even internally sourced data can be an attack vector if tampered with.
**Prevention:** Always pass dynamic query parameters as a dictionary via the `params` argument in `requests.get()`. This ensures the library handles proper URL encoding and parameter merging securely.
