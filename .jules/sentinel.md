...
**Vulnerability:** Constructing API URLs with f-strings and query parameters is susceptible to HTTP Parameter Pollution and unexpected injection if the parameters contain special characters (like `&` or `=`).
**Learning:** Using `requests.get()` with `params` argument automatically handles URL encoding safely and merges dictionary parameters avoiding concatenation bugs.
**Prevention:** Prefer using the `params` dictionary for all URL queries in `requests.get()`.
