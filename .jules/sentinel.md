...
## 2024-05-24 - API Parameter Injection Prevention
**Vulnerability:** Python scripts (like update.py) were constructing API request URLs using f-strings and string concatenation to insert external data directly into the URL string.
**Learning:** This approach is vulnerable to API parameter injection, where unescaped characters like `&` or spaces in the injected variables can alter the URL structure, change intended request parameters, or manipulate the API call behavior.
**Prevention:** Always use the `requests.get(url, params=dict)` method, which automatically and safely URL-encodes parameters and merges them with any existing query strings in the base URL.
