...
## 2025-05-15 - API Parameter Injection
**Vulnerability:** `update.py` used Python f-strings to inject dynamic tag data into the Gelbooru API URL (e.g., `url = f"{API_URL}&tags={tag}"`). This allowed potential injection of arbitrary API parameters or invalid URL construction if the tag contained unescaped characters like ampersands (`&`) or question marks (`?`).
**Learning:** URL string concatenation with unvalidated user/external data is a common vector for API parameter injection or Server-Side Request Forgery (SSRF) bypasses. The `requests` library automatically and safely URL-encodes dictionaries passed to the `params` argument.
**Prevention:** Never use f-strings or string concatenation to build URL query strings with dynamic data. Always use the `params` argument provided by the `requests` library (e.g., `requests.get(url, params={'tags': tag})`) to ensure parameters are properly encoded and securely merged into the request.
