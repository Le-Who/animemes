...
## 2024-06-28 - URL Injection / HTTP Parameter Pollution
**Vulnerability:** URL string concatenation in `requests.get()` allowed URL injection and HTTP Parameter Pollution.
**Learning:** Constructing URLs manually with f-strings using untrusted input (tags) allows attackers to inject arbitrary query parameters (e.g., overriding `limit` or `json`).
**Prevention:** Always use the `params` argument dictionary in `requests.get()` to securely encode and manage query parameters.
