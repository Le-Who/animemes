...
## 2024-04-20 - [HTTP Parameter Pollution Prevention]
**Vulnerability:** Manual string interpolation was used to construct URLs with user-controlled input (like `tag`), exposing the API requests to HTTP Parameter Pollution and injection risks.
**Learning:** Python `requests.get()` correctly merges pre-existing query strings in a base URL with dynamic parameters passed as a dictionary, providing an automatic and secure way to build complex queries without manual escaping.
**Prevention:** Always use the `params` dictionary argument in `requests` instead of manually interpolating variables into URL strings with f-strings.
