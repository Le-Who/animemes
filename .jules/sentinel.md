...
## 2024-05-24 - URL Parameter Injection
**Vulnerability:** URL Parameter Injection via string concatenation in requests.get
**Learning:** Constructing URLs with f-strings for API requests allows malicious or malformed tags to inject arbitrary query parameters (e.g., modifying `limit` or `page`).
**Prevention:** Always use the `params` dictionary in `requests.get()` to safely URL-encode external variables instead of constructing URL strings manually.
