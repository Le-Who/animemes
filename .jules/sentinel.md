...
## 2024-05-24 - Prevent URL Parameter Injection in Python
**Vulnerability:** URL Parameter Injection via f-strings
**Learning:** Constructing URLs with f-strings directly embeds user input, which can contain unescaped characters like `&` or `=`. This allows attackers to manipulate the HTTP request's query string, potentially injecting arbitrary parameters or altering the request's intended behavior.
**Prevention:** Always use the `params` dictionary argument in `requests.get()` to ensure query parameters are safely URL-encoded and automatically merged with any existing query parameters in the base URL.
