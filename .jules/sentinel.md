...
## 2024-06-03 - URL Injection Vulnerability in update.py
**Vulnerability:** URL Injection in backend script via raw string concatenation with unsanitized user/database input.
**Learning:** Constructing API endpoint URLs with `f-strings` using raw string injection from JSON (`f"...&tags={tag}..."`) allows special characters (like `&` and `=`) to alter the GET request structure.
**Prevention:** Always use safe URL encoding libraries (like `urllib.parse.quote`) or rely on a request library's built-in parameter serialization (e.g. `requests.get(url, params=...)`) instead of raw string formatting.
