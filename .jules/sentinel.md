...
## 2026-03-14 - Prevent HTTP Parameter Pollution in API calls
**Vulnerability:** Unencoded URL interpolation in backend scripts (like `update.py`) can lead to HTTP Parameter Pollution if a variable contains special characters like `&` or `=`.
**Learning:** Always URL-encode variables before interpolating them into API request URLs, even in internal scripts.
**Prevention:** Use `urllib.parse.quote()` in Python or `encodeURIComponent()` in JavaScript to sanitize inputs before placing them in a URL query string.
