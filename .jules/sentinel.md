...
## 2024-05-24 - Prevent API Parameter Injection in Python
**Vulnerability:** Constructing API URLs using f-strings with unsanitized parameters introduces API parameter injection risks.
**Learning:** Raw string concatenation fails to URL-encode special characters like `&` or `=`, allowing the request query structure to be manipulated.
**Prevention:** Always use the `params` dictionary in `requests.get()` to automatically handle URL encoding of dynamically passed parameters.
