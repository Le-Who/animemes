...
## 2026-10-28 - API Parameter Injection via f-strings
**Vulnerability:** URL parameters in `update.py` were constructed using f-strings and string concatenation without URL encoding. This allowed parameter injection and URL corruption if tags contained special characters like `&` or `=`.
**Learning:** Using f-strings to inject external inputs into query strings is unsafe. It bypasses URL encoding, leading to potential parameter overriding and invalid HTTP requests.
**Prevention:** Always use the `params` dictionary provided by the `requests` library. It automatically and securely handles URL encoding, combining parameters safely.
