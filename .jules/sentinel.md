...
**Vulnerability:** Constructing external API URLs using f-strings with untrusted input (`tags={tag}`) creates an HTTP Parameter Pollution/Injection risk if the input contains `&` or other URI-breaking characters.
**Learning:** Even when querying safe 3rd party APIs, injecting variables directly into the query string bypasses standard URL encoding and can manipulate the request structure.
**Prevention:** Always use the `params` dictionary provided by request libraries (like `requests.get(url, params=...)`) which safely URL-encodes all parameters and merges them correctly.
