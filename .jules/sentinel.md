...
## 2026-04-19 - HTTP Parameter Pollution via f-strings
**Vulnerability:** HTTP Parameter Pollution via unsafe f-string URL construction in Python (`update.py`). Passing unescaped variables directly into URL query strings allows attackers to inject arbitrary query parameters.
**Learning:** `requests.get()` safely merges query parameters passed via the `params` dictionary with any pre-existing query strings already present in the base URL.
**Prevention:** Always use the `params` dictionary argument in `requests.get()` to handle dynamic query parameters instead of manually interpolating them using f-strings or `urllib.parse.quote()`.
