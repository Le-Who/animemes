...
## 2024-05-19 - API Parameter Injection in update.py
**Vulnerability:** The `update.py` script constructs API URLs by concatenating unvalidated `tag` strings from `characters.json` directly into the URL using f-strings, leading to API parameter injection.
**Learning:** String concatenation for URLs is a fragile anti-pattern. Even when reading from local JSON files, data should be treated as untrusted, especially in projects where users can submit PRs.
**Prevention:** Always use the `params` dictionary in `requests.get()` to let the library safely encode and merge query parameters.
