...
## 2024-05-24 - Prevent API Parameter Injection in update.py
**Vulnerability:** URL parameters were constructed using f-strings, allowing potential API parameter injection if tag values contained ampersands or other control characters.
**Learning:** String concatenation for URLs is insecure and error-prone.
**Prevention:** Always use the `params` argument in `requests.get()` to safely encode and append query parameters.
