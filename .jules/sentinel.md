...
## 2026-04-04 - HTTP Parameter Pollution in API Calls
**Vulnerability:** `update.py` constructed API URLs by manually concatenating user-controlled variables (tags) via f-strings, exposing the application to HTTP Parameter Pollution.
**Learning:** The Python `requests.get()` function safely URL-encodes and merges query parameters passed via the `params` dictionary with any pre-existing query strings present in the base URL.
**Prevention:** Always use the `params` dictionary in `requests` rather than manually interpolating query parameters with f-strings or `urllib.parse.quote()`.
