...
## 2025-03-30 - HTTP Parameter Pollution via URL Interpolation
**Vulnerability:** `update.py` constructed API URLs by interpolating the `tag` variable directly into the URL string using f-strings. This could lead to broken requests or HTTP Parameter Pollution if a tag contained special characters (like `&` or `=`).
**Learning:** Dynamically building query strings with f-strings bypasses proper URL encoding and parameter separation, creating injection vectors.
**Prevention:** Always use the `params` dictionary argument in `requests.get()` to pass dynamic query parameters. The library safely encodes values and merges them into the URL.
