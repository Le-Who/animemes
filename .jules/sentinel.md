...
## 2025-02-28 - Prevent HTTP Parameter Pollution in API Requests
**Vulnerability:** Constructing URLs using f-string interpolation (e.g., `f"...&tags={tag}"`) exposes the script to HTTP Parameter Pollution. If a tag contains unescaped characters like `&` or `=`, it could manipulate the API query.
**Learning:** `requests.get()` safely merges query parameters passed via the `params` dictionary with pre-existing query strings in the base URL, automatically handling URL encoding.
**Prevention:** Always use the `params` argument in `requests.get()` to pass dynamic query parameters instead of manually interpolating them into the URL string.
