...
## 2026-06-12 - Parameter Injection in API Requests
**Vulnerability:** URL parameters for `requests.get()` were constructed using simple string formatting (`f"{API_URL}&tags={tag}"`). If the `tag` variable contained URL control characters like `&` or `=`, it could override subsequent parameters or inject unintended ones (e.g., overriding `json=1`).
**Learning:** Even in backend scripts acting as clients, unencoded data combined into URLs can cause HTTP Parameter Pollution (HPP). Python's `requests` library automatically URL-encodes the `params` dictionary.
**Prevention:** Always use the `params` dictionary argument in `requests.get()` rather than manually constructing query strings with string formatting.
