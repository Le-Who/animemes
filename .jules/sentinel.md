...
**Vulnerability:** API query strings in Python scripts were constructed using f-strings and string concatenation without URL encoding.
**Learning:** This exposes the application to parameter injection if user input contains characters like `&` or `=`.
**Prevention:** Always use the `params` dictionary in `requests.get()` to safely pass query parameters, as it handles URL encoding and merges parameters automatically.
