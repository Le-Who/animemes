...
## 2026-07-16 - URL Parameter Injection via String Formatting
**Vulnerability:** Constructing URLs via string formatting (e.g., `url = f"{API_URL}&tags={tag}..."`) instead of using the `requests` library's `params` argument allowed for URL Parameter Injection if `tag` contained unescaped characters like `&` or `=`.
**Learning:** Always delegate URL query string encoding to the HTTP client library (like `requests`) by passing a `params` dictionary, rather than manually interpolating variables into the URL string.
**Prevention:** Use `params` argument in `requests.get()` and let the library handle safe URL encoding.
