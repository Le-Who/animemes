...
## 2024-05-24 - API Parameter Injection
**Vulnerability:** Constructing URL queries using f-strings allows parameter injection.
**Learning:** `requests.get` properly handles URL encoding and appending when parameters are passed in the `params` dictionary, while f-strings are vulnerable if user data contains special characters like `&`.
**Prevention:** Always use the `params` dictionary in `requests.get` to merge dynamic query parameters safely.
