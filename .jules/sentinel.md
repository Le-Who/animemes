...
## 2026-02-10 - Unsafe-Inline CSP Removal
**Vulnerability:** The application relied on `unsafe-inline` in its Content Security Policy (CSP) for styles, allowing potential XSS via CSS injection if an attacker could inject a `<style>` tag or `style` attribute.
**Learning:** Extracting internal CSS to an external file allows the removal of `unsafe-inline`, significantly hardening the application against XSS attacks that leverage style injection.
**Prevention:** Avoid inline styles and `<style>` blocks. Use external stylesheets and classes to maintain a strict CSP.
