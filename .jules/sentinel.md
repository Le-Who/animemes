...
## 2025-10-24 - URL Injection via Unsafe String Concatenation in Requests
**Vulnerability:** `update.py` constructed API request URLs by directly embedding external variables (tags) using f-strings (e.g., `url = f"{API_URL}&tags={tag}"`). This exposes the application to HTTP Parameter Pollution and URL Injection, allowing malicious tags to manipulate query parameters.
**Learning:** Using f-strings to build URLs bypasses URL encoding. External inputs can contain characters like `&` or `=` that alter the structure of the HTTP request, potentially bypassing filters or altering application logic.
**Prevention:** Always use the `params` dictionary provided by `requests.get()` to pass query string parameters. The `requests` library will safely merge and URL-encode the parameters, mitigating injection risks.
