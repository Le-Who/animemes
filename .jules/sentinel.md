...
## 2025-02-18 - URL Injection via Unsanitized Parameters
**Vulnerability:** `update.py` constructed API URLs using f-strings with raw tag parameters, potentially allowing URL injection or HTTP parameter pollution if a malicious tag payload was introduced.
**Learning:** Even internal helper scripts fetching external data must sanitize parameters, as data files like `characters.json` could be manipulated or external sources might return malicious tag names that get re-queried.
**Prevention:** Always use the `params` dictionary in `requests.get()` rather than manual URL concatenation to ensure parameters are properly safely URL-encoded.
