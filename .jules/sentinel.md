...
## 2024-05-24 - Fix HTTP Parameter Pollution in update.py
**Vulnerability:** Unsafe URL construction using string formatting for API requests in `update.py` allowed potential HTTP parameter injection if a tag contained special characters (like `&` or `=`).
**Learning:** Hardcoding query parameters using string concatenation or f-strings bypasses safe URL encoding.
**Prevention:** Always use the `params` argument in `requests.get` to safely and automatically encode URL query parameters.
