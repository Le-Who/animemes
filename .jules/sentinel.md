...
## 2024-05-24 - Prevent HTTP Parameter Pollution via URL f-strings
**Vulnerability:** Constructing API URLs via f-strings risks HTTP Parameter Pollution if inputs contain unescaped characters like `&` or `=`.
**Learning:** Always pass parameters explicitly via the `requests.get(params=...)` dictionary, which automatically URL-encodes user input safely.
**Prevention:** Use `params` kwarg in `requests.get()` instead of manual URL interpolation.
