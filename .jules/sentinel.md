...
## 2026-06-16 - Prevent URL Injection via String Formatting
**Vulnerability:** URL variables were directly interpolated into URL strings instead of using `params`.
**Learning:** Always use `requests.get(url, params=...)` to let the requests library safely URL-encode the parameters to avoid parameter injection/pollution.
**Prevention:** Use `params` for all GET requests that take variable input.
