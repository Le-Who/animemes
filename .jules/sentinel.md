...
## 2026-07-09 - Prevent URL Injection in API requests
**Vulnerability:** HTTP Parameter Pollution and URL Injection possible due to constructing API URLs using f-strings and manual concatenation with unsanitized tags.
**Learning:** The requests library can safely merge and URL-encode query parameters passed via the params dictionary, even if the base URL already contains query string parameters. This prevents attackers from injecting arbitrary query parameters.
**Prevention:** Always use the params dictionary in requests.get() to safely encode external variables rather than using manual string concatenation or f-strings.
