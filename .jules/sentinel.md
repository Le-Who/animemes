...
## 2024-05-24 - API Parameter Injection via String Formatting
**Vulnerability:** URL query parameters in `update.py` were built using unsafe string formatting (e.g., `f"{API_URL}&tags={tag}"`), allowing parameter injection via unescaped characters like `&` in the `tag` variable.
**Learning:** Python's `requests.get()` safely URL-encodes and merges dictionary properties passed to the `params` keyword argument.
**Prevention:** Avoid f-strings for URL parameters; always use the `params` argument in `requests.get()` to prevent URL manipulation.
