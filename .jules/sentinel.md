...
## 2024-05-24 - URL Injection / HTTP Parameter Pollution Prevention
**Vulnerability:** Constructing API URLs with f-strings allows URL injection if user input contains special characters like `&`.
**Learning:** External tags were directly appended to URLs, allowing parameter manipulation.
**Prevention:** Always use the `params` argument in `requests.get()` to safely URL-encode all query parameters automatically.
