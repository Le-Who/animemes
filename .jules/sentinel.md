...
## 2024-06-26 - URL Injection / Parameter Pollution in API Requests
**Vulnerability:** URL variables were directly interpolated into URL strings using f-strings, leading to potential URL Injection or HTTP Parameter Pollution.
**Learning:** `requests` will safely merge and URL-encode the `params` dictionary even if the base URL already contains query parameters, preventing injection issues.
**Prevention:** Always use the `params` dictionary in `requests.get()` to safely encode external variables instead of constructing URLs via string concatenation or f-strings.
