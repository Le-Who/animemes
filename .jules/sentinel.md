...
## 2025-02-18 - HTTP Parameter Pollution in API Requests
**Vulnerability:** String concatenation was used to construct URLs for API requests in `update.py`. This allowed for potential HTTP Parameter Pollution and invalid requests if `tag` variables contained special characters like `&` or `=`.
**Learning:** `requests.get()` safely merges query parameters passed via the `params` dictionary with pre-existing query strings in the base URL, safely handling URL encoding and preventing injection.
**Prevention:** Avoid interpolating dynamic values into URLs with f-strings. Always use the `params` argument in `requests` to handle query parameters.
