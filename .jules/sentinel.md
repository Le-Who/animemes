...
## 2024-05-08 - Prevent parameter injection in HTTP requests
**Vulnerability:** Constructing API URLs with f-strings and string concatenation allows for parameter injection and malformed URLs if inputs contain special characters.
**Learning:** Python's `requests` library automatically handles URL encoding safely when parameters are passed via the `params` dictionary. Using f-strings circumvents this protection.
**Prevention:** Always use the `params` dictionary in `requests.get()` instead of manual URL concatenation.
