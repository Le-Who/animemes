...
## 2024-05-18 - Parameter Injection via String Formatting
**Vulnerability:** Constructing URLs via f-strings with unsanitized user input (`tag`) in `update.py` allowed for API parameter injection.
**Learning:** Even internal or non-database queries are susceptible to injection attacks if untrusted data alters the structure of the request payload or URL path/query strings.
**Prevention:** Always use safe serialization, such as the `params` dictionary in `requests.get()`, which handles URL encoding safely and automatically, rather than string concatenation or interpolation.
