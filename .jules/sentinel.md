...
## 2025-02-19 - URL Injection in requests
**Vulnerability:** Constructing URLs for `requests.get` using string concatenation and f-strings (e.g., `f"&tags={tag}"`) is vulnerable to HTTP Parameter Pollution or URL Injection if the variable contains unexpected characters like `&` or `=`.
**Learning:** `requests` will safely merge and URL-encode the `params` dictionary even if the base URL already contains query parameters. It handles encoding internally, preventing parameters from breaking out and altering the query string structure.
**Prevention:** Always use the `params` dictionary parameter in `requests` methods to safely encode external variables, rather than constructing URL strings manually.
