...
## 2026-05-02 - API Parameter Injection
**Vulnerability:** `update.py` used f-strings to concatenate the `tag` directly into the `url` for `requests.get()`. This allows a malformed tag to inject arbitrary URL query parameters.
**Learning:** Even in local scripts, dynamically constructing URLs with string interpolation can lead to parameter injection if the input contains characters like `&` or `=`.
**Prevention:** Always use the `params` dictionary in `requests.get()` to safely merge dynamically passed query parameters. `requests` handles the correct URL encoding automatically.
