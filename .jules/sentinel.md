...
## 2025-03-09 - URL Injection via F-strings in Requests
**Vulnerability:** Constructing URLs via f-strings and manual query string appending (e.g., `f"&tags={tag}"`) allowed URL Injection/HTTP Parameter Pollution if `tag` contained characters like `&` or `=`.
**Learning:** External or user-provided data must be properly URL-encoded before being appended to API endpoints to prevent injection into unexpected query parameters or paths.
**Prevention:** Always use the `params` dictionary argument in the `requests` library when making HTTP requests with dynamic data. It safely handles URL encoding and merges parameters correctly.
