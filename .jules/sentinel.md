...
## 2025-05-18 - API Parameter Injection via String Concatenation
**Vulnerability:** URL string concatenation in `update.py` allowed potential API parameter injection if tags contained unencoded characters (like `&`).
**Learning:** Constructing URLs dynamically with string formatting (f-strings) bypasses URL encoding and parameter sanitization.
**Prevention:** Always use the `params` dictionary in `requests.get()` to let the library handle safe URL encoding and parameter merging securely.
