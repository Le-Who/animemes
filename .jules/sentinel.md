...
## 2026-04-05 - Prevent HTTP Parameter Pollution in API Requests
**Vulnerability:** Constructing API URLs using string interpolation (f-strings) in `update.py` exposed the script to HTTP Parameter Pollution and URL injection if tags contained unescaped characters like `&` or `=`.
**Learning:** Even internal scripts that construct external API requests must sanitize parameters. The Python `requests` library provides built-in, safe parameter serialization.
**Prevention:** Always pass dynamic query parameters as a dictionary to the `params` argument in `requests.get()` instead of manually interpolating them into the URL string.
