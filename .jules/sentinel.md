...
## 2024-04-12 - Fix HTTP Parameter Pollution Risk
**Vulnerability:** HTTP Parameter Pollution and injection risk due to manual f-string interpolation for URL construction with dynamic tag input.
**Learning:** Using f-strings to inject dynamic inputs directly into URLs bypasses URL encoding, allowing parameter injection if the input contains characters like `&` or `=`.
**Prevention:** Always use the `params` dictionary argument in `requests.get()` to safely construct URLs with dynamic parameters, as the requests library automatically handles correct URL encoding and merging.
