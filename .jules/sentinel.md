...
## 2025-03-22 - HTTP Parameter Pollution via F-Strings
**Vulnerability:** In `update.py`, dynamic query parameters were manually concatenated into URLs using f-strings (e.g., `url = f"{API_URL}&tags={tag}..."`). If a tag contains special characters like `&` or `=`, it could alter the API request structure (HTTP Parameter Pollution), leading to unintended queries or errors.
**Learning:** Constructing URLs via manual string interpolation is fragile and insecure. It bypasses proper URL encoding of parameter values.
**Prevention:** Always use the dedicated `params` argument in libraries like `requests` (e.g., `requests.get(url, params={'tags': tag})`) to safely encode and serialize dynamic query data.
