...
## 2024-07-06 - Prevent HTTP Parameter Pollution in API calls
**Vulnerability:** URL injection / HTTP Parameter Pollution due to constructing API request URLs using f-strings with unvalidated tags.
**Learning:** External data used in API requests must be URL-encoded, otherwise malicious tags with & or = can alter the request structure.
**Prevention:** Always use the params dictionary in requests.get() to ensure variables are safely URL-encoded.
