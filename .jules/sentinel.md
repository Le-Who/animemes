...
## 2025-04-22 - Query Parameter Injection via F-Strings
**Vulnerability:** `update.py` interpolated user-controlled tags directly into API URLs using f-strings (e.g., `f"&tags={tag}"`). This could allow query parameter injection if tags contained special characters like `&` or `#`.
**Learning:** Never interpolate variables directly into URL query strings. Always rely on URL encoding mechanisms provided by HTTP libraries.
**Prevention:** Use the `params` dictionary argument in `requests.get()` to let the library safely encode all query parameters automatically.
