...
## 2024-05-24 - Prevent Parameter Injection in API Requests
**Vulnerability:** URL parameters in `update.py` were constructed using Python f-strings, allowing potential URL manipulation and parameter injection if malicious tags were provided.
**Learning:** String interpolation for URLs is insecure. Always use the request library's built-in parameter handling to automatically URL-encode values.
**Prevention:** Use the `params` argument in `requests.get()` to safely construct queries, which safely escapes characters and prevents manipulation of the request structure.
