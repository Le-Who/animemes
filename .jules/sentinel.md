...
## 2026-06-23 - Parameter Injection in API Requests
**Vulnerability:** External input (`tag`) was directly interpolated into URLs using f-strings in `update.py` before being passed to `requests.get()`. This allowed HTTP Parameter Pollution or Server-Side Request Forgery if the input contained characters like `&`.
**Learning:** Even when the base URL points to a trusted domain, directly embedding unsanitized input into the URL string can alter the request semantics (e.g., bypassing limits or changing query structure).
**Prevention:** Always use the `params` dictionary in HTTP client libraries like `requests` to automatically handle safe URL encoding of query parameters.
