...
## 2025-05-20 - HTTP Parameter Pollution via F-Strings
**Vulnerability:** URL construction using raw f-strings (`f"&tags={tag}"`) allowed user-controlled `tag` values to inject arbitrary query parameters (like `&limit=1000` or altering other API directives) via HTTP Parameter Pollution.
**Learning:** Never interpolate user input directly into URLs, especially in API integrations where parameter manipulation can lead to SSRF-like behavior, denial of service, or unintended data disclosure.
**Prevention:** Always use the `params` dictionary argument in `requests.get()`. This ensures the library properly URL-encodes keys and values, preventing injection of control characters like `&` or `=`.
