...
// OPTIMIZATION: Reuse Intl.NumberFormat to avoid recreating it on every frame/update
...
function animateValue(obj, start, end, duration) {
    ...
    const step = (timestamp) => {
        ...
        // Bolt: Only update DOM if value actually changed to prevent layout thrashing
        const val = Math.floor(progress * (end - start) + start);
        if (val !== lastVal) {
            obj.textContent = numberFormatter.format(val);
            lastVal = val;
        }
