...
// Bolt: Cache Intl.NumberFormat to prevent GC overhead in animation loop
...
function animateValue(obj, start, end, duration) {
    ...
    let lastVal = null; // Bolt: Track last value to prevent redundant DOM updates
    const step = (timestamp) => {
        if (!startTimestamp) startTimestamp = timestamp;
        const progress = Math.min((timestamp - startTimestamp) / duration, 1);

        // Bolt: Only format and update DOM when integer value changes
