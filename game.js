...
function next() {
    ...
    announce(`Раунд ${score + 1}. Выберите популярного персонажа.`);
    preloadNext();
}
...
function vote(side) {
    ...
    if (isWin) {
        ...
        announce(`Верно! ${rightItem.name}: ${getVal(rightItem).toLocaleString()}. Счет: ${score}`);
    } else {
        score = 0;
        if (side === 'left') { els.c1.classList.add('lose'); els.c2.classList.add('win'); }
        else { els.c2.classList.add('lose'); els.c1.classList.add('win'); }
        announce(`Неверно! ${rightItem.name}: ${getVal(rightItem).toLocaleString()}. Счет сброшен.`);
    }
