...
def get_data(tag):
    try:
        ...
        # 🛡️ Sentinel: Securely build the URL using the params parameter to prevent URL injection
        params = {
        ...
        if '@attributes' in data and 'count' in data['@attributes']:
             ...
             try:
                tag_api_url = "https://gelbooru.com/index.php?page=dapi&s=tag&q=index&json=1"
                # 🛡️ Sentinel: Securely build the URL using the params parameter
                tag_params = {'names': tag}
                tag_resp = requests.get(tag_api_url, headers=headers, params=tag_params, timeout=10)
