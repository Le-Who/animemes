...
def get_data(tag):
    try:
        ...
        # Security: Use requests params to prevent URL injection/HTTP parameter pollution
        params = {
        ...
        if '@attributes' in data and 'count' in data['@attributes']:
             ...
             try:
                tag_api_base = "https://gelbooru.com/index.php?page=dapi&s=tag&q=index&json=1"
                tag_params = {'names': tag}
                tag_resp = requests.get(tag_api_base, params=tag_params, headers=headers, timeout=10)
