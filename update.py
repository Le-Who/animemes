...
def get_data(tag):
    try:
        ...
        # 🛡️ Sentinel: Use params dict to prevent HTTP Parameter Pollution
        params = {
        ...
        if '@attributes' in data and 'count' in data['@attributes']:
             ...
             try:
                tag_url_base = "https://gelbooru.com/index.php?page=dapi&s=tag&q=index&json=1"
                # 🛡️ Sentinel: Safe query parameter construction
                tag_params = {'names': tag}
                tag_resp = requests.get(tag_url_base, headers=headers, params=tag_params, timeout=10)
