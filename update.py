...
def get_data(tag):
    try:
        ...
        # Security: Pass tags and limit securely via params dictionary
        # to prevent HTTP Parameter Pollution and ensure correct URL encoding
        params = {
        ...
        if '@attributes' in data and 'count' in data['@attributes']:
             ...
             try:
                ...
                # Security: Prevent parameter injection via dict-based params
                tag_params = {'names': tag}
                tag_resp = requests.get(tag_base_url, params=tag_params, headers=headers, timeout=10)
