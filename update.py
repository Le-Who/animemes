...
def get_data(tag):
    try:
        ...
        url = f"{API_URL}&limit=20"
        print(f"Fetching: {tag}...")
        
        response = requests.get(url, headers=headers, timeout=15, params={'tags': f"{tag} sort:score:desc"})
        ...
        if '@attributes' in data and 'count' in data['@attributes']:
             ...
             try:
                tag_url = f"https://gelbooru.com/index.php?page=dapi&s=tag&q=index&json=1"
                tag_resp = requests.get(tag_url, headers=headers, timeout=10, params={'names': tag})
