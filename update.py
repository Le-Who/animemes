...
def get_data(tag):
    try:
        ...
        # Security: Prevent HTTP Parameter Pollution by using params dict
        request_params = {
            'tags': f"{tag} sort:score:desc",
            'limit': 20
        }
        response = requests.get(API_URL, headers=headers, params=request_params, timeout=15)
        ...
        if '@attributes' in data and 'count' in data['@attributes']:
             ...
             try:
                ...
                tag_resp = requests.get(tag_url, headers=headers, params={'names': tag}, timeout=10)
