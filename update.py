...
def get_data(tag, session=None):
    try:
        ...
        # Bolt: Reuse session to avoid TCP handshake overhead on each request
        req_func = session.get if session else requests.get
        response = req_func(url, headers=headers, timeout=15)
        ...
        if '@attributes' in data and 'count' in data['@attributes']:
             ...
             try:
                ...
                tag_resp = (session.get if session else requests.get)(tag_url, headers=headers, timeout=10)
...
def update_database():
    ...
    # Bolt: Initialize a requests.Session to persist TCP connections across multiple API requests
