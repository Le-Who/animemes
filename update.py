...
def get_data(tag):
    try:
        ...
        # Security: Prevent URL parameter injection by using params dict
        params = {
        ...
        if '@attributes' in data and 'count' in data['@attributes']:
             ...
             try:
                # Security: Prevent URL parameter injection by using params dict
                tag_api = "https://gelbooru.com/index.php?page=dapi&s=tag&q=index&json=1"
                tag_params = {'names': tag}
                tag_resp = requests.get(tag_api, headers=headers, params=tag_params, timeout=10)
