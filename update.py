...
def get_data(tag, session=requests):
    try:
        ...
        # ⚡ Bolt: Use session for connection pooling if provided
...
def update_database():
    ...
    # ⚡ Bolt: Use Session to pool connections across multiple API requests
    with requests.Session() as s:
        for char in db:
            # Небольшая пауза перед каждым запросом (Anti-Spam)
            time.sleep(random.uniform(1.0, 3.0))
            
            count, img = get_data(char['tag'], session=s)
